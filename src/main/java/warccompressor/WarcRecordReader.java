...
public class WarcRecordReader extends RecordReader<LongWritable, Text> {
    private static final Log LOG = LogFactory.getLog(WarcRecordReader.class);

    private CompressionCodecFactory compressionCodecs = null;
    private long start;
    private long pos;
    private long end;
    private WarcLineReader in;
    ...
    public WarcRecordReader(byte[] recordDelimiter) {
    ...
    public void initialize(InputSplit genericSplit,
        ...
        if (isCompressedInput()) {
            ...
            if (codec instanceof SplittableCompressionCodec) {
                ...
                in = new WarcLineReader(cIn, job, recordDelimiterBytes);
                start = cIn.getAdjustedStart();
                end = cIn.getAdjustedEnd();
                filePosition = cIn;
            } else {
                in = new WarcLineReader(codec.createInputStream(fileIn, decompressor), job,
                        recordDelimiterBytes);
                filePosition = fileIn;
            }
        } else {
            fileIn.seek(start);
            in = new WarcLineReader(fileIn, job, recordDelimiterBytes);
            filePosition = fileIn;
        }
